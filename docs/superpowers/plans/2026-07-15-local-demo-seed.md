# Local Demo Seed Script Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** One command (`npm run seed:demo`, after `supabase db reset`) fills the local Supabase instance with a realistic, lived-in demo club — safe to put on camera for YouTube walkthroughs, with fixed known login credentials.

**Architecture:** A standalone Node script (`web/scripts/seed-demo.ts`, run via `tsx`) uses `@next/env` to load `.env.local` the same way Next.js does, then a Supabase service-role client (bypasses RLS) to: create 5 auth users, 1 club, 18 drills, 2 session plans, 1 coaching group with membership, and a community thread + DM with messages. An idempotency guard checks for the demo club before doing anything, so the intended workflow is always `supabase db reset && npm run seed:demo`.

**Tech Stack:** `tsx` (new devDependency), `@next/env` (already available via the `next` package), `@supabase/supabase-js` service-role client.

## Global Constraints

- **Local only.** No new Vercel environment, no new Supabase project, no production changes (per approved spec).
- Not designed to be re-run against a dirty database — the idempotency guard exits early with an instruction to reset first, rather than silently duplicating data.
- No automated test — this is a dev-tooling script, not shipped product code (explicit spec non-goal). Verification is manual: run the script, log in with the printed credentials, confirm each screen is populated.
- `clubs` table has no wildcard `select('*')` grant for anon/authenticated roles (per `web/supabase/migrations/075_clubs_hide_stripe_columns.sql`) — irrelevant here since the script uses the service-role client, which bypasses RLS and column grants entirely, but don't copy this script's client setup into any user-facing code path.

---

### Task 1: Add `tsx` and the `seed:demo` npm script

**Files:**
- Modify: `web/package.json`

**Interfaces:**
- Produces: a `seed:demo` script and a `tsx` devDependency that Task 2's script relies on to run.

- [ ] **Step 1: Install `tsx` as a devDependency**

Run: `cd web && npm install -D tsx`
Expected: `package.json` and `package-lock.json` update with `tsx` under `devDependencies`.

- [ ] **Step 2: Add the npm script**

In `web/package.json`, in the `"scripts"` block, add a new line after `"test:watch": "vitest"`:
...
git commit -m "chore: add tsx and seed:demo script for local demo data"
```

---

### Task 2: Write `web/scripts/seed-demo.ts`

**Files:**
- Create: `web/scripts/seed-demo.ts`

**Interfaces:**
- Consumes: `loadEnvConfig` from `@next/env`; `createClient` from `@supabase/supabase-js`; env vars `NEXT_PUBLIC_SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` from `.env.local`.
- Produces: no exports — this is a standalone executable script invoked via `npm run seed:demo`.

Exact schema this script writes to (confirmed against `web/supabase/migrations/`):
- `profiles`: `id, username, display_name, bio, coaching_level, role, club_id, club_role, subscription_tier` (row auto-created by a DB trigger on `auth.users` insert — update, don't insert).
- `clubs`: `id, name, slug, created_by, subscription_tier, max_members, max_groups`.
- `drill_categories`: pre-seeded by `web/supabase/seed.sql` (slugs: `attack`, `defence`, `kicking`, `fitness`, `handling`, `set-plays`, `warm-up`) — read, don't write.
- `drills`: `title, description, category_id, canvas_json, author_id, difficulty, age_group, player_count, is_public, club_id, approval_status`.
- `drill_ratings`: `drill_id, user_id, rating, comment`.
- `session_plans`: `title, coach_id, drills_order (jsonb array of {drill_id, duration_minutes, notes}), total_duration, is_shared, group_id`.
- `coaching_groups`: `name, club_id, created_by`.
- `group_invitations` (membership table — a user is "in" a group via an `accepted` row here): `group_id, user_id, invited_by, status, group_role`.
- `conversations`: `title, type ('community'|'dm'), created_by`.
...
```bash
git add web/scripts/seed-demo.ts
git commit -m "feat: add local demo seed script for screen recording"
```
...
- [ ] **Step 1: Run a full reset and seed**

Run (from `web/`, with the local Supabase stack running via `supabase start`):
...
Expected: the script prints "Demo data seeded successfully." followed by 5 email/password rows, ending with the club summary line. No error output.

- [ ] **Step 2: Re-run to confirm the idempotency guard**

Run: `npm run seed:demo` again (without resetting).
Expected: script exits immediately with `Demo club "Ashfield Rhinos RLFC" already exists. Run "supabase db reset" first, then re-run this script.` and does not create duplicate users or drills.

- [ ] **Step 3: Spot-check the app**

Run: `npm run dev`, then in the browser:
- Log in as `demo.headcoach@18thman.local` / `Demo1234!`. Confirm the dashboard loads with the club name visible.
- Open the drill library — confirm ~18 drills appear across categories, with visible canvas previews (not blank).
- Open Sessions — confirm both session plans appear, one marked shared.
- Open the coaching group — confirm all 4 club coaches are listed as members.
- Open Community — confirm the seeded thread and its 3 messages appear.
- Open Chat/DMs — confirm the DM with Jordan Pike and its 4 messages appear.
- Log out, log in as `demo.freecoach@18thman.local` / `Demo1234!`. Confirm this account shows no club and free-tier upgrade prompts (no seeded club drills visible to them beyond public ones).
