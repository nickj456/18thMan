...
list (shadcn `Card` + custom radio buttons — no `radio-group` primitive exists yet in
`components/ui/`, so this sub-project adds one via the standard shadcn CLI pattern, matching
`checkbox.tsx`'s existing style). If this question was already answered (via Back), the
previously selected option is preselected.
...
**Back** (`actions.ts` → `goBack(attemptId, currentQuestionOrder)`): navigates to the previous
question by order; no data mutation, just a redirect (the previous answer is still saved and
preselects on render).
...
| 19 | You're planning for a finals campaign. | Set out the technical standards the team needs to hit to compete at that level. *(Teacher)* | Build the tactical game plan specifically for the sides you'll likely face. *(Game Manager)* | Get very deliberate about logistics — travel, prep, timing — for the bigger occasion. *(Organiser)* | Focus on keeping the team grounded and connected under the extra pressure. *(Culture Builder)* |
