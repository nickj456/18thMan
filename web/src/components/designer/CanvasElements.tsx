...
import { Circle, Rect, Arrow, Line, Text, Group, Shape } from 'react-konva'
...
function RugbyBall({ el, selected, onSelect, onChange }: ElementProps) {
  ...
  return (
    ...
    >
      {/* Main body — white oval with slight point at each end */}
      <Shape
        sceneFunc={(ctx, shape) => {
          ctx.beginPath()
          ctx.moveTo(-rx, 0)
      ...
      <Shape
        sceneFunc={(ctx) => {
          ctx.save()
          // Clip to the ball shape so chevrons don't bleed outside
          ctx.beginPath()
          ctx.moveTo(-rx, 0)
          ctx.bezierCurveTo(-rx * 0.6, -ry, rx * 0.6, -ry, rx, 0)
          ctx.bezierCurveTo(rx * 0.6, ry, -rx * 0.6, ry, -rx, 0)
          ...
          // Inner orange chevron — left side
          ctx.beginPath()
          ctx.moveTo(-rx + 7, -5)
      ...
      {/* Lace stitching — vertical centre */}
      {[-8, -4, 0, 4, 8].map((dx) => (
        <Line
          key={dx}
          points={[dx, -3, dx, 3]}
